...
<table id="cart" class="table table-hover table-condensed" style="width:100%">
		...
		foreach($result as $row)
		{
			...
			<td data-th="Product">
							...
							<td style="text-align:right">$'.number_format($funciones->getPrecioCant($row['id'],$tipousuario,$row['cant']),2,',','.').'</td>
<td style="text-align:right">$'.number_format($funciones->getPrecioCant($row['id'],$tipousuario,$row['cant'])*$row['cant'],2,',','.').'</td>
						</tr>
					';
		...
		?>
					...
					<tfoot>
						<tr>
							...
							<td></td>
							<td></td>
							<td colspan="2" style="text-align:right;border-top:1px solid #ccc;"><strong>Total $<?php echo number_format($funciones->getTotalPedido($_REQUEST['id']),2,',','.');?></strong></td>
