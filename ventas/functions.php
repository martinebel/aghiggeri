...
class Funciones
{
	...
	{
		$total=0;
		$stmt = $this->conexion->prepare("select temp_pedidos_vendedor.cant,temp_pedidos_vendedor.idproducto from temp_pedidos_header_vendedor inner join temp_pedidos_vendedor on temp_pedidos_vendedor.id=temp_pedidos_header_vendedor.idpedido where clave='".$idpedido."'");
		 $stmt->execute();
		$result = $stmt->fetchAll(); 
		foreach($result as $row)
		{
	...
	{
		$total=0;
		$stmt = $this->conexion->prepare("select count(idpedido) as total from temp_pedidos_header_vendedor where idcliente='".$idcliente."' and clave <>'".$uid."'");
		 $stmt->execute();
		$result = $stmt->fetchAll(); 
		foreach($result as $row)
		{
