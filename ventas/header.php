...
session_start();
 ...
 if( !isset($_SESSION['cid']) ){header('Location: index.php');die();}
...
{
	$stmt = $dbh->prepare("update temp_pedidos_vendedor set cant=".$cantarray[$i]." where itemno=".$key." and id='".$idpedido."'");
        $stmt->execute();
	$i++;
}
$stmt=$dbh->prepare("update temp_pedidos_header_vendedor set total='".$funciones->getTotalTempPedido($_SESSION['uid'],$_SESSION['tipousuario'])."' where idpedido=".$idpedido);
    $stmt->execute();

  }

  if($_POST['action']=="nuevopedido"){
    $uid=generateSession();
    $_SESSION['uid']=$uid;

    $stmt=$dbh->prepare("insert into temp_pedidos_header_vendedor (`idpedido`, `fecha`, `idcliente`, `clave`, `total`, `idvendedor`, `estado`)
     values (NULL,'".date('Y-m-d')."','".$_POST['idcliente']."','".$uid."',0,'".$_SESSION['cid']."','')");
    $stmt->execute();
    $idpedido=$dbh->lastInsertId();

 $stmt=$dbh->prepare("select * from clientes where idcliente='".$_POST['idcliente']."'");
   $stmt->execute();
   $result2 = $stmt->fetchAll();
    ...
    <div class="container">
      <ul class="list-inline pull-left hidden-sm hidden-xs">
       <!--<li><span class="text-primary"><i class="fa fa-phone" aria-hidden="true"></i> Atención al Cliente </span>0362 – 4433100 - 4451555</li>-->
        <li><span class="text-primary">MODO DE PRUEBA </span></li>
      </ul>
      <ul class="topBarNav pull-right">
        <li>Usuario: <?php echo $_SESSION["cname"];?></li>
      </ul>
      <?php
     /* if( isset($_SESSION['uid']) ){
      echo '<ul class="topBarNav pull-right">
		...
		$totalitems=$stmt->rowCount();
          ...
          echo '<a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="false"> <i class="fa fa-shopping-basket mr-5"></i> <span class="hidden-xs">
 ...
 <!--=========MIDDEL-TOP_BAR============-->
  ...
  <div class="row display-table">
    <div class="col-sm-3 vertical-align text-left hidden-xs">
      <a href="home.php"> <img style="height: 74px;
    margin-top: -15px;
    margin-left: -28px;
...
<nav class="navbar navbar-main navbar-default" role="navigation" style="opacity: 1;">
    ...
    if( !isset($_SESSION['uid']) ){
     ...
     <li><a href="home.php">Pedidos Pendientes</a></li>';
   }
   else
   {
    echo '<li><a href="home.php">Pedido Actual</a></li>';
    //NOVEDADES
